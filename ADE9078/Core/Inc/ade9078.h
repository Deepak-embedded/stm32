 ...
 */
#include<stdint.h>
#ifndef INC_ADE9078_H_
#define INC_ADE9078_H_
...
typedef struct{
	uint32_t AngleReg_VA_VB;
	uint32_t AngleReg_VB_VC;
	uint32_t AngleReg_VA_VC;
	uint32_t AnlgeReg_VA_IA;
	uint32_t AngleReg_VB_IB;
	uint32_t AngleReg_VC_IC;
	uint32_t AngleReg_IA_IB;
	uint32_t AngleReg_IB_IC;
	uint32_t AngleReg_IA_IC;
}AngleRegs_xx_xx;
...
typedef struct{
	int32_t Line_PeriodReg_A;
	int32_t Line_PeriodReg_B;
	int32_t Line_PeriodReg_C;

}Line_period_Regs;

typedef struct{
	int32_t Active_PowerReg_A;
	int32_t Active_PowerReg_B;
	int32_t Active_PowerReg_C;

}Active_pow_Regs;

typedef struct{
	int32_t Reactive_PowerReg_A;
	int32_t Reactive_PowerReg_B;
	int32_t Reactive_PowerReg_C;
}Reactive_pow_Regs;

typedef struct{
	int32_t Apparent_PowerReg_A;
	int32_t Apparent_PowerReg_B;
	int32_t Apparent_PowerReg_C;
}Apparent_pow_Reg;

typedef struct{
	double Active_Power_A;
	double Active_Power_B;
	double Active_Power_C;
}Active_pow;

typedef struct{
	double Reactive_Power_A;
	double Reactive_Power_B;
	double Reactive_Power_C;
}Reactive_pow;

typedef struct{
	double Apparent_Power_A;
	double Apparent_Power_B;
	double Apparent_Power_C;
}Apparent_pow;

typedef struct{
	uint32_t Power_FactorReg_A;
	uint32_t Power_FactorReg_B;
	uint32_t Power_FactorReg_C;
}Power_Factor_Regx;

typedef struct{
	double Power_Factor_A;
	double Power_Factor_B;
	double Power_Factor_C;
}Power_Factorx;
...
#define CS_DISABLE 0
#define CS_ENABLE  1
#define CS_LOW 		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, CS_DISABLE)
#define CS_HIGH     HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, CS_ENABLE)
#define RESET_LOW	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, CS_DISABLE)
#define RESET_HIGH	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, CS_ENABLE)

#define ADE_ID 128
#define ANGL_RESOLUTION   0.036// (1/(CLKIN/24))/(1/f_line) × 360°.//CLKIN=12,f_line=1/50Hz
...
#define RSTDONE() ({ \
    uint32_t value; \
    ADE9078_read_Regs_Data(STATUS1_32, &value, BYTE_32); \
    (value>>16)&0x01; \
})
...
#define VERSION_16 0x4FE //Reset: 0x0080 Access: R

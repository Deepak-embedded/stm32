...
float get_supply(double voltageReading, eMode_t conversion_mode){
	...
	case TC_T :
        ...
        float tc_uV = voltageReading * 1000000.0f;
        float tCJ = 1.0f;
        if (!isfinite(tCJ)) tCJ = 25.0f;
        float cj_uV = K_type_C_to_uV(tCJ);
        float total_uV = tc_uV + cj_uV;
        float tHot = K_type_uV_to_C(total_uV);
        retVal = tHot;
        break;

...
flag_flashUpdateEPROM_Frequent;
...
unsigned int flag_flashSaveRecipe_WaitCounter=10,flag_flashUpdateEPROM_General_WaitCounter=10,flag_flashUpdateEPROM_AI_Calibration_WaitCounter=10,flag_flashUpdateEPROM_Schedule_WaitCounter=10,
		flag_flashUpdateEPROM_Modbus_Quary_Detail_WaitCounter=10,flag_flashUpdateEPROM_PCBPLC_GENERAL_REG_WaitCounter=0,flag_flashUpdateEPROM_Frequent_WaitCounter;
...
W25Q_STATE W25Q_ReadSByte(i8_t *buf, u8_t pageShift, u32_t pageNum) {
	...
	u8_t data;
	W25Q_STATE state = MX25L_ReadRaw(&data, 1, rawAddr);
	if (state != W25Q_OK)
		return state;
	memcpy(buf, &data, 1);
	return W25Q_OK;
...
W25Q_STATE W25Q_ReadSWord(i16_t *buf, u8_t pageShift, u32_t pageNum) {
	...
	u32_t rawAddr = page_to_addr(pageNum, pageShift);
	u8_t data[2];
	W25Q_STATE state = MX25L_ReadRaw(data, 2, rawAddr);
	if (state != W25Q_OK)
		return state;
	memcpy(buf, data, 2);
	return W25Q_OK;
...
W25Q_STATE W25Q_ReadSLong(i32_t *buf, u8_t pageShift, u32_t pageNum) {
	...
	u32_t rawAddr = page_to_addr(pageNum, pageShift);
	u8_t data[4];
	W25Q_STATE state = MX25L_ReadRaw(data, 4, rawAddr);
	if (state != W25Q_OK)
		return state;
	memcpy(buf, data, 4);
	return W25Q_OK;
...
W25Q_STATE W25Q_ReadData(u8_t *buf, u16_t len, u8_t pageShift, u32_t pageNum) {
	if (pageNum >= PAGE_COUNT || len == 0 || len > 256 || pageShift > 256 - len)
		return W25Q_PARAM_ERR;
	u32_t rawAddr = page_to_addr(pageNum, pageShift);
	return MX25L_ReadRaw(buf, len, rawAddr);
}
...
W25Q_STATE W25Q_ProgramSByte(i8_t buf, u8_t pageShift, u32_t pageNum) {
	...
	u8_t data;
	memcpy(&data, &buf, 1);
	return MX25L_PageProgram_QPI(&data, 1, rawAddr);
}
...
W25Q_STATE W25Q_ProgramSWord(i16_t buf, u8_t pageShift, u32_t pageNum) {
	...
	u8_t data[2];
	memcpy(data, &buf, 2);
	return MX25L_PageProgram_QPI(data, 2, rawAddr);
}
...
W25Q_STATE W25Q_ProgramSLong(i32_t buf, u8_t pageShift, u32_t pageNum) {
	...
	u8_t data[4];
	memcpy(data, &buf, 4);
	return MX25L_PageProgram_QPI(data, 4, rawAddr);
}
...
W25Q_STATE W25Q_ProgramData(u8_t *buf, u16_t len, u8_t pageShift, u32_t pageNum) {
	if (pageNum >= PAGE_COUNT || len == 0 || len > 256 || pageShift > 256 - len)
		return W25Q_PARAM_ERR;
	u32_t rawAddr = page_to_addr(pageNum, pageShift);
	return MX25L_PageProgram_QPI(buf, len, rawAddr);
}
...
W25Q_STATE Erase_RECsector(unsigned int startAddress,int total_no_sec)
{
	...
	{
		FLASH_ret = MX25L_EraseSector_QPI(startAddress + (no_sec*4096));
...
W25Q_STATE W25Q_Erase_Write_One_Sector(u8_t *buf, u16_t len, u32_t pageShift){
	...
	{
		status1=MX25L_EraseSector_QPI(current_addr); //SECTOR_COUNT
		if(status1!= W25Q_OK){
			return W25Q_PARAM_ERR;
		}
...
//	}
	...
	do
	{
		status2 = MX25L_PageProgram_QPI(buf, current_size, current_addr);
		if(status2!= W25Q_OK){
			return W25Q_PARAM_ERR;
		}
		HAL_Delay(3);
...
void ExtFlash_Read_RuntimePara(unsigned char makeDefault)
{
	...
	MX25L_ReadRaw((u8_t*) &g_flashRunTimeParaSturct, sizeof(g_flashRunTimeParaSturct), HISTORY_DATA_RUN_TIME_PARA_START_ADDRESS);
...
void ExtFlash_ReadHistoricalDataLogFromFlash(unsigned int pageCounter,unsigned char *buffer)
{
	...
	MX25L_ReadRaw((u8_t*) &tflashDataSturct, sizeof(tflashDataSturct), HistoryDataAddressPointer);

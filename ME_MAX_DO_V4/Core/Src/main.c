...
volatile uint32_t uart1_last_rx_time = 0;
volatile uint32_t uart3_last_rx_time = 0;
volatile uint32_t uart1_process_start = 0;
volatile uint32_t uart3_process_start = 0;


volatile uint32_t uart11_last_rx_time = 0;


volatile uint8_t uart1_recovering = 0;
volatile uint8_t uart3_recovering = 0;

uint32_t uart1_last_recovery = 0;
uint32_t uart3_last_recovery = 0;
uint32_t frame_delay = 4;

uint8_t buf_A; // DIP SWITCH ID

volatile uint8_t RxIndex1 = 0, RxIndex2 = 0;
uint8_t RxByte1, RxByte3;
uint32_t lastByteTime1 = 0, lastByteTime3 = 0;

uint8_t slave_id;
uint8_t buf_A; // DIP SWITCH ID
uint32_t selectedBaudRate;
uint16_t gpioVal;
uint8_t  lastGpioVal = 0xFF;
...
/* USER CODE END Includes */
static uint8_t prev_slave_id = 0;
...
void UpdateUartBaudRate(void)
{
	...
	if (selectedBaudRate != previousBaud)
	{
		...
		UART_SetBaudRate(&huart3, selectedBaudRate);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) {
       HAL_TIM_Base_Stop_IT(&htim2); // Stop timer until next packet
        ...
        uint16_t len = 64 -  __HAL_DMA_GET_COUNTER(huart1.hdmarx);
        uint16_t len1 = 64 - __HAL_DMA_GET_COUNTER(huart3.hdmarx);
        if (len >= 8) { // Minimum Modbus RTU frame size
     	  ...
     	  memcpy(RxData1,UART1_rx_buffer,len);
    	   ...
    	   uart3_frame_ready = 1;
    	    uart3_rx_size = len1;
    	    memcpy(RxData,UART3_rx_buffer,len1);
       }
    }
}
...
//    	}
    ...
    if (uart1_frame_ready)
    {
            ...
            uint16_t crc_calc = crc16_1(uart1_buf, len - 2);
            uint16_t crc_rx   = uart1_buf[len-2] | (uart1_buf[len-1] << 8);

            if (crc_calc != crc_rx) continue;
            if (uart1_buf[1] != 0x01 && uart1_buf[1] != 0x0f) continue;
    ...
    if (uart3_frame_ready){
			  ...
			  if (crc_calc != crc_rx) continue;
			  if ((uart3_buf[1] != 0x01) && (uart3_buf[1] != 0x0f) ) continue;

			  startAddr = (uart3_buf[2] << 8) | uart3_buf[3];
			  numCoils  = (uart3_buf[4] << 8) | uart3_buf[5];
...
void UART3_Recover(void)
{
    ...
    // Check all errors first
    if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_ORE)) error = 1;
    if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_FE))  error = 1;
    if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_NE))  error = 1;
    if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_PE))  error = 1;
    ...
    if (huart->Instance == USART1)
    {
        if (Size >= 6 && Size <= sizeof(RxData1))
        {
            if (RxData1[0] == slave_id && !uart1_frame_ready)
            {
                ...
                uart1_rx_size = Size;
                uart1_last_rx_time = HAL_GetTick();
                uart1_process_start = uart1_last_rx_time;
